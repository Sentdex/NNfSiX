...
public class Script6 : MonoBehaviour
{
    ...
    // These are prefabs of objects made to mark out the positions for the various classes in the spiral. Coloured circles is what I used.
    public GameObject class1;
    public GameObject class2;
    public GameObject class3;
    // Game Object to act as parent for the above prefabs, should be child of the canvas.
    ...
    void Update()
    {
        if (dataReady)
        {
            // Pass data from CreateData method through 1st layer, and then activation function, then 2nd layer and its activation function
    ...
    public void CreateData()
    {
        ...
        for (int i = 1; i < classes + 1; i++)
        {
            ...
            {
                X[index + ((i - 1) * 100)] = new float[] { r[index + 1] * Mathf.Sin(t[index] * 2.5f), r[index + 1] * Mathf.Cos(t[index] * 2.5f) };
                switch (i)
                {
                    case 1:
