...
template <class L> 
auto pretty_print(const L& list) -> 
	decltype(std::declval<L>().list, void()) { 
	std::cout << "[ ";
	std::copy(std::begin(list), std::end(list), std::ostream_iterator<double>(std::cout, " "));
	std::cout << "]\n";
} 
...
auto zip(const M& m, const N& n)
{
	constexpr auto m_s = decltype(m.size)::get();
	constexpr auto n_s = decltype(n.size)::get();
        constexpr auto size = std::min(m_s, n_s);
        List<std::tuple<typename M::value_type, typename N::value_type>, size> zipped;
	for (auto [it, mIt, nIt] = std::tuple{std::begin(zipped), std::begin(m), std::begin(n)};
			(mIt != std::end(m) && nIt != std::end(n));
		++mIt, ++nIt, ++it) {
		*it = {*mIt, *nIt};
	}
        return zipped;
...
// numpy randn
template <class L>
auto randn(L& list, double factor) ->
		decltype(std::declval<L>().list, void()) 
{
...
auto dot(M& inputs, N& weights, O& biases)
{
	...
	std::transform(std::begin(inputs), std::end(inputs), std::begin(outputs),
			[width, &zipped_weights_and_biases](const auto& input){
		List<double, width> outputLine;
		std::transform(std::begin(zipped_weights_and_biases), std::end(zipped_weights_and_biases), std::begin(outputLine),
				[&input, &zipped_weights_and_biases](const auto& zipped_weights_and_biasesLine){
			/* std::cout << "Weights Line "; pretty_print(weightsLine); */
			/* std::cout << "Input : ";pretty_print(input); */
			double result = std::inner_product(std::begin(input), std::end(input), std::begin(std::get<0>(zipped_weights_and_biasesLine)), 0.0f)+std::get<1>(zipped_weights_and_biasesLine);
			return result;
		});
		return outputLine;
	});
...
template<std::size_t inputs_width, std::size_t inputs_height, std::size_t n_neurons>
class DenseLayer {
public:
	DenseLayer()
	{		
