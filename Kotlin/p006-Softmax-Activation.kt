...
fun main() {
    ...
    var dense1 = Layer_Dense(2, 3)
    var activation1 = Activation_ReLU()

    var dense2 = Layer_Dense(3, 3)
    var activation2 = Activation_SoftMax()
...
private class Activation_SoftMax {
    var output: MutableList<MutableList<Double>> = mutableListOf()

    fun forward(inputs: MutableList<MutableList<Double>>) {
        var exp_values: MutableList<MutableList<Double>> = inputs.map { row -> row.map { e -> exp(e - row.max())}.toMutableList()}.toMutableList()

        output = exp_values.map { row -> row.map { e -> e / row.sum()}.toMutableList()}.toMutableList()
    }
}
...
private fun MutableList<Double>.max(): Double {
    var max_value: Double = -10.0;
    for (i in this) {
        if (i > max_value) {
            max_value = i
        }
...
// get sum of 1D array
private fun MutableList<Double>.sum(): Double {
    var sum: Double = 0.0
    for (i in this) {
        sum += i
    }
